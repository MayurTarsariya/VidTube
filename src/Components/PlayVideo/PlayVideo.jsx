        ...
        const fetchVideoData = async () => {
            try {
                ...
                if (videoItem) {
                    ...
                    const commentsDetailsUrl = `https://youtube.googleapis.com/youtube/v3/commentThreads?part=snippet%2Creplies&maxResults=100&videoId=${videoId}&key=${API_KEY}`;
...
useEffect(()=>{
    ...
    if (!apiData || !channelData || !commentData) {
        return (
            <div className={isDark ? 'loading loaddark' : 'loading'}><h3>Loading...</h3></div>
        );
    }

    return (
        <div className={isDark ? "play-video videodark" : "play-video"}>
            <iframe
